...
boruta_df_small <- readRDS("/datos/home/marh/OSTEOSARCOMA/Recurrence_bin.RDS")

boruta_df_small <- boruta_df_small %>% 
  dplyr::select(-EVENT_DAYS)
...
x_data <- boruta_df_small[, setdiff(colnames(boruta_df_small), "EVENT_STAT")]

x_data <- as.data.frame(apply(x_data, 2, as.numeric))

y_data <- boruta_df_small$EVENT_STAT
...
x_log2_filtered <- x_log2[, colnames(x_log2) %in% names(keep_expression)[keep_expression == TRUE]]
...
# Define path
out_path <- "/datos/home/marh/OSTEOSARCOMA/Final_signatures/Recurrence_bin"
...
saveRDS(boruta.signature, paste0(out_path, "/boruta_rec_bin_20.rds"))

# 2. Save the final model object
saveRDS(final_boruta_decided, paste0(out_path, "/final_boruta_decided_rec_bin_20.rds"))
...
write.csv(final_gene_names, paste0(out_path, "/final_gene_rec_bin_20.csv"), row.names = FALSE)

# 5. Export the importance values (the numbers used in your plot)
stats <- attStats(boruta.signature)
write.csv(stats, paste0(out_path, "/gene_importance_full_stats_rec_bin_20.csv"))
